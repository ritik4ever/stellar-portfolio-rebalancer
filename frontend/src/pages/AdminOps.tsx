...
function AdminGuard({ children }: { children: React.ReactNode }) {
    ...
    const [state, setState] = useState<AdminGuardState>('authorized')
...
export default function AdminOps() {
    ...
    const services = (systemData?.services ?? systemData?.riskManagement?.services) ?? {}

    const queueEntries = Object.entries(queueData?.queues ?? {})
                ...
                {!isLoading && !isEmpty && (
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        ...
                        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    {(['healthy', 'unhealthy', 'idle', 'lagging'] as const).map((key) => (
                                        ...
                                        <div key={key} className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                                            ...
                                            <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-white">
                                                ...
                                                {(workersData?.summary as Record<string, number> | undefined)?.[key] ?? 0}
                            ...
                            <div className="mt-4 flex items-center justify-between rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-500 dark:border-gray-700 dark:text-gray-400">

                                <span>Total workers</span>

                                <span className="text-lg font-semibold text-gray-900 dark:text-white">{(workersData?.summary?.total ?? 0).toLocaleString()}</span>
                        ...
                        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                            ...
                            {!systemData ? (
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    ...
                                    <SystemBadge label="Price Feeds" value={services.priceFeeds ?? false} />

                                    <SystemBadge label="Risk Management" value={services.riskManagement ?? false} />

                                    <SystemBadge label="WebSockets" value={services.webSockets ?? false} />

                                    <SystemBadge label="Auto Rebalance" value={services.autoRebalancing ?? false} />

                                    <SystemBadge label="Stellar Network" value={services.stellarNetwork ?? false} />

                                    <SystemBadge label="Event Indexer" value={services.contractEventIndexer ?? false} />
