...
const AssetCard: React.FC<AssetCardProps> = ({ asset, price, isLoading = false }) => {
        ...
        return (
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-gray-700 animate-pulse">
                ...
                <div className="space-y-1 sm:space-y-2">
                    ...
                    <div className="w-full bg-gray-300 dark:bg-gray-700 rounded-full h-1.5 sm:h-2 mt-1 sm:mt-2" />
                </div>
            </div>
        )
    }
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-gray-700">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
                <div className="flex items-center min-w-0 flex-1">
                    ...
                    <div className="ml-2 sm:ml-3 min-w-0">
                        ...
                        <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{asset.value}% allocation</p>
                        {asset.issuer && (
                ...
                <div className={`flex items-center flex-shrink-0 ${isPositive ? 'text-green-500' : isNegative ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'}`}>
                    {isPositive && <TrendingUp className="w-3 sm:w-4 h-3 sm:h-4 mr-0.5 sm:mr-1" data-testid="trend-up" />}
                    {isNegative && <TrendingDown className="w-3 sm:w-4 h-3 sm:h-4 mr-0.5 sm:mr-1" data-testid="trend-down" />}
                    {isNeutral && <span className="w-3 sm:w-4 h-3 sm:h-4 mr-0.5 sm:mr-1 flex items-center justify-center font-bold text-xs sm:text-sm" data-testid="trend-neutral">-</span>}
            ...
            <div className="space-y-1 sm:space-y-2">
                <div className="flex justify-between">
                    <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium">{priceLabel}</span>
                    <div className="text-right">
                        <span className="text-xs sm:text-sm font-semibold dark:text-gray-200 block">{formattedPrice}</span>
                        {priceCaption && (
                            <span className="text-[10px] sm:text-xs text-gray-400 dark:text-gray-500 block leading-tight">{priceCaption}</span>
                        )}
                    </div>
                </div>

                <div className="flex justify-between">
                    <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Value</span>
                    <span className="text-xs sm:text-sm font-medium dark:text-gray-200">${asset.amount.toLocaleString()}</span>
                </div>

                <div className="flex justify-between">
                    <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Target</span>
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">{asset.value}%</span>
                </div>
