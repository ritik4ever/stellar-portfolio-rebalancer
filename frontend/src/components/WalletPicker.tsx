...
export const WalletPicker: React.FC<WalletPickerProps> = ({ onConnect, onError, onDisconnect }) => {
  ...
  const detectInstalled = (walletType: string): boolean => {
    ...
    switch (walletType) {
      ...
      case 'hana': {
        const stellar = window.stellar as any
        return !!window.hana || String(stellar?.provider ?? '').toLowerCase().includes('hana') || String(stellar?.platform ?? '').toLowerCase().includes('hana')
      }
