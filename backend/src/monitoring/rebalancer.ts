...
export class RebalancingService {
                ...
                try {
                    ...
                    if (needsRebalance) {
                        ...
                        if (portfolio.autoRebalance) {
                            await this.executeAutomaticRebalance(portfolio.id)
                        }
                    }
                } catch (error) {
    ...
    private async executeAutomaticRebalance(portfolioId: string) {
        try {
            const prices = await this.reflectorService.getCurrentPrices()
            const highVolatility = this.detectHighVolatility(prices)

            if (highVolatility) {
                logger.warn(`Skipping auto-rebalance for portfolio ${portfolioId} due to high volatility`)
                this.notifyClients(portfolioId, 'rebalance_skipped_volatility')
                return
            }

            const result = await this.stellarService.executeRebalance(portfolioId)
            logger.info(`Auto-rebalance executed for portfolio ${portfolioId}`, { result })

            this.notifyClients(portfolioId, 'auto_rebalanced', result)

        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error)
            logger.error(`Auto-rebalance failed for portfolio ${portfolioId}:`, { error: errorMessage })
            this.notifyClients(portfolioId, 'rebalance_failed', { error: errorMessage })
        }
    }

    private detectHighVolatility(prices: Record<string, any>): boolean {
        const volatilityThreshold = 10

        for (const [asset, data] of Object.entries(prices)) {
            if (Math.abs(data.change) > volatilityThreshold) {
                logger.warn(`High volatility detected: ${asset} moved ${data.change}%`)
                return true
            }
        }
