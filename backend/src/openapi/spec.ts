...
const spec: Record<string, any> = {
    ...
    paths: {
        ...
        '/api/admin/events/replay': {
            post: {
                tags: ['Rebalance history'],
                summary: 'Replay contract events',
                description: 'Re-index contract events from a given ledger. Admin only.',
                security: [{ adminAuth: [] }],
                parameters: [
                    {
                        name: 'from_ledger',
                        in: 'query',
                        required: true,
                        description: 'The ledger sequence number to start replaying from',
                        schema: {
                            type: 'integer',
                            minimum: 1
                        }
                ...
                responses: {
                    '200': { description: 'Replay result', content: { 'application/json': { schema: { $ref: '#/components/schemas/ApiEnvelope' } } } },
                    '400': { description: 'Validation error', content: { 'application/json': { schema: { $ref: '#/components/schemas/ApiError' } } } },
                    '401': { description: 'Unauthorized' },
                    '422': { description: 'Replay failed', content: { 'application/json': { schema: { $ref: '#/components/schemas/ApiError' } } } },
                    '500': { description: 'Internal error', content: { 'application/json': { schema: { $ref: '#/components/schemas/ApiError' } } } },
                },
            },
        },
