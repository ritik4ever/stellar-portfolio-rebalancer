...
import {
    ...
    recordRebalanceEventSchema,
    autoRebalancerControlSchema
} from './validation.js'
...
router.post('/portfolio', writeRateLimiter, validateRequest(createPortfolioSchema), async (req, res) => {
    try {
...
router.post('/portfolio/:id/rebalance', writeRateLimiter, validateRequest(rebalancePortfolioSchema), async (req, res) => {
    try {
        const portfolioId = req.params.id
...
router.get('/rebalance/history', async (req, res) => {
    try {
        ...
        const history = await rebalanceHistoryService.getRebalanceHistory(
            ...
            limit,
            {
                eventSource: (source === 'all' ? undefined : source) as 'offchain' | 'simulated' | 'onchain' | undefined,
                startTimestamp,
                endTimestamp
            }
        )
...
router.post('/rebalance/history', validateRequest(recordRebalanceEventSchema), async (req, res) => {
    try {
...
router.post('/auto-rebalancer/start', requireAdmin, validateRequest(autoRebalancerControlSchema), (req, res) => {
    try {
        if (!autoRebalancer) {
...
router.post('/auto-rebalancer/stop', requireAdmin, validateRequest(autoRebalancerControlSchema), (req, res) => {
    try {
        if (!autoRebalancer) {
            return res.status(500).json({
                success: false,
...
router.post('/auto-rebalancer/force-check', requireAdmin, validateRequest(autoRebalancerControlSchema), async (req, res) => {
    try {
        if (!autoRebalancer) {
            return res.status(500).json({
                success: false,
