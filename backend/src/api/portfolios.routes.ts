...
import {
  ...
  portfolioAttributionQuerySchema,
} from "./validation.js";
...
portfoliosRouter.get(
  ...
  validateQuery(portfolioExportQuerySchema),
  async (req: Request, res: Response) => {
    try {
      const portfolioId = req.params.id;
      ...
      if (
        authConfig.enabled &&
        (!req.user || portfolio.userAddress !== req.user.address)
      ) {
        return fail(
          res,
          403,
      ...
      if (!databaseService.hasFullConsent(portfolio.userAddress)) {
        return fail(
          ...
          403,
          "FORBIDDEN",
          "Active consent is required before exporting portfolio data",
        );
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const portfolio = (await portfolioStorage.getPortfolio(portfolioId)) as
        ...
        | undefined;
      if (!portfolio) return fail(res, 404, "NOT_FOUND", "Portfolio not found");
      const { prices, feedMeta } =
        await reflectorService.getCurrentPricesWithMeta();
      ...
      try {
        ...
        if (
          ...
          return fail(
            ...
            403,
            "FORBIDDEN",
            "You can only rebalance your own portfolio",
          );
        }
...
portfoliosRouter.get(
  ...
  validateQuery(portfolioAttributionQuerySchema),
  async (req: Request, res: Response) => {
    try {
      const portfolioId = req.params.id;
      ...
      if (req.query.from && !from) {
        return fail(res, 400, "VALIDATION_ERROR", "Invalid from date");
      }
      if (req.query.to && !to) {
        return fail(res, 400, "VALIDATION_ERROR", "Invalid to date");
      }
      if (from && to && new Date(from).getTime() > new Date(to).getTime()) {
        return fail(res, 400, "VALIDATION_ERROR", "from must be before to");
      }
      ...
      return ok(res, attribution, {
        meta: { contributions: attribution.contributions.length },
      });
    } catch (error) {
      logger.error("Failed to fetch attribution", {
        error: getErrorObject(error),
        portfolioId: req.params.id,
      });
      return fail(res, 500, "INTERNAL_ERROR", getErrorMessage(error));
