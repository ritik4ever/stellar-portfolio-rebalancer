...
import {
  ...
  isPortfolioCheckWorkerRunning,
} from "./queue/workers/portfolioCheckWorker.js";
import {
  startRebalanceWorker,
  stopRebalanceWorker,
  isRebalanceWorkerRunning,
} from "./queue/workers/rebalanceWorker.js";
import {
  startAnalyticsSnapshotWorker,
  stopAnalyticsSnapshotWorker,
  isAnalyticsSnapshotWorkerRunning,
} from "./queue/workers/analyticsSnapshotWorker.js";
...
try {
  ...
  logger.info(
    "[STARTUP-CONFIG] Validation successful",
    buildStartupSummary(startupConfig),
  );
} catch (error) {
...
app.options("*", (req, res) => {
  ...
  res.setHeader(
    ...
    "Content-Type, Authorization, Accept, Origin, X-Requested-With, X-Public-Key, X-Message, X-Signature",
  );
  res.status(204).end();
});
...
app.get("/health/ready", async (req, res) => {
  try {
    const dbReady = databaseService.healthCheck();
    ...
    const indexerReady = indexerStatus.enabled
      ? indexerStatus.running && !indexerStatus.lastError
      : true;
    ...
    try {
      ...
      const result = await reflector.testApiConnectivity();
      extReady = result.success;
    } catch (e) {
      extReady = false;
    }

    const systemReady = dbReady && redisReady && workersReady && !!indexerReady;
    ...
    const responsePayload = {
      ...
      components: {
        ...
        queue: redisReady ? "up" : "down",
        workers: workersReady ? "up" : "down",
        indexer: indexerReady ? "up" : indexerStatus.lastError || "down",
        external: extReady ? "up" : "down",
      },
    };
...
app.use((req, res, next) => {
  ...
  const matchesLegacy = LEGACY_API_PREFIXES.some(
...
try {
  ...
  logger.info(
    "[REBALANCING-SERVICE] Monitoring service started (queue-backed)",
  );
} catch (error) {
...
server.listen(port, async () => {
  ...
  if (shouldStartAutoRebalancer) {
    try {
      logger.info(
        "[AUTO-REBALANCER] Starting automatic rebalancing service...",
      );
      await autoRebalancer.start();
      logger.info(
        "[AUTO-REBALANCER] Automatic rebalancing service started successfully",
      );
      ...
      logger.error(
        "[AUTO-REBALANCER] Failed to start automatic rebalancing service",
        { error },
      );
    }
  } else {
    logger.info(
      "[AUTO-REBALANCER] Automatic rebalancing disabled in development mode",
    );
    logger.info(
      "[AUTO-REBALANCER] Set ENABLE_AUTO_REBALANCER=true to enable in development",
    );
  }
