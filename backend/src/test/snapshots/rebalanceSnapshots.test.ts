...
const SNAPSHOTS_DIR = join(__dirname, 'fixtures')
...
function loadOrGenerateSnapshots(): RebalanceSnapshot[] {
  if (existsSync(SNAPSHOT_FILE)) {
    const raw = readFileSync(SNAPSHOT_FILE, 'utf-8')
    return JSON.parse(raw) as RebalanceSnapshot[]
  }
...
function regenerateSnapshots(): void {
  const snapshots = SNAPSHOT_FIXTURES.map((fixture) => {
    const result = computeRebalanceSnapshot(fixture.input)
    return {
      ...fixture,
...
describe('Rebalance Simulation Snapshots', () => {
  const snapshots = loadOrGenerateSnapshots()

  for (const snapshot of snapshots) {
    it(`produces deterministic output for "${snapshot.name}"`, () => {
      const result = computeRebalanceSnapshot(snapshot.input)

      expect(result.totalValue).toBe(snapshot.expectedTotalValue)
      expect(result.trades).toEqual(snapshot.expectedTrades)
    })
  }

  it('all snapshot fixtures match regenerated output', () => {
    const regenerated = SNAPSHOT_FIXTURES.map((fixture) => {
      const result = computeRebalanceSnapshot(fixture.input)
      return {
        ...fixture,
        expectedTrades: result.trades,
    ...
    expect(regenerated).toEqual(snapshots)
  })
})

export { computeRebalanceSnapshot, regenerateSnapshots, SNAPSHOT_FIXTURES }
