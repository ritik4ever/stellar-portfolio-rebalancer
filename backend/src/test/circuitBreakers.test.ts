...
describe('CircuitBreakers', () => {
    ...
    describe('recovery path', () => {
        ...
        it('recovers after cooldown expires and allows rebalancing', () => {
            ...
            const duringCooldown = CircuitBreakers.checkCooldownPeriod(lastRebalance, 1)
            ...
            const afterCooldown = CircuitBreakers.checkCooldownPeriod(lastRebalance, 1)
        ...
        it('recovers market conditions after volatility normalizes', () => {
            ...
            const highVolResult = CircuitBreakers.checkMarketConditions(highVolatilityPrices)
        ...
        it('recovers after correlation normalizes', () => {
            ...
            const correlatedResult = CircuitBreakers.checkMarketConditions(correlatedPrices)
    ...
    describe('end-to-end lifecycle', () => {
        ...
        it('complete lifecycle: trigger → cooldown → recovery', () => {
            ...
            const triggerResult = CircuitBreakers.checkMarketConditions(highVolPrices)
            expect(triggerResult.safe).toBe(false)

            // Step 2: Verify cooldown blocks rebalance
            const lastRebalance = new Date('2026-01-01T10:00:00.000Z').toISOString()
            const cooldownResult = CircuitBreakers.checkCooldownPeriod(lastRebalance, 1)
            ...
            const recoveryCooldownResult = CircuitBreakers.checkCooldownPeriod(lastRebalance, 1)
        ...
        it('handles multiple assets with staggered recovery', () => {
            ...
            const btcTrigger = CircuitBreakers.checkMarketConditions(btcVolatilePrices)
            ...
            const ethTrigger = CircuitBreakers.checkMarketConditions(ethVolatilePrices)
            ...
            const combinedResult = CircuitBreakers.checkMarketConditions(combinedPrices)
