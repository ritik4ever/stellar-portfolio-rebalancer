...
function authHeader(address: string): Record<string, string> {
  const token = jwt.sign({ sub: address, type: "access" }, JWT_SECRET, {
    expiresIn: "15m",
  });
...
describe("Portfolio CRUD API Integration Tests with JWT Authentication", () => {
  ...
  beforeAll(() => {
    ...
    const testDir = join(
      tmpdir(),
      `stellar-portf-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe("GET /api/portfolio/:id/attribution", () => {
    let portfolioId: string;
    ...
    it("returns empty attribution for portfolios with no history", async () => {
      const res = await request(app)
      ...
      expect(res.body.data.contributions.length).toBe(0);
      expect(res.body.data.totalReturnPercent).toBe(0);
    });

    it("returns attribution with from/to query parameters", async () => {
