...
vi.mock("../services/databaseService.js", () => ({
  databaseService: {
    ...
    getContractEventIndexerState: vi.fn(),
    saveContractEventIndexerState: vi.fn(),
    resetContractEventIndexerState: vi.fn(),
...
describe("contractEventIndexer", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.CONTRACT_EVENT_SCHEMA_VERSION = String(
      BACKEND_CONTRACT_EVENT_SCHEMA_VERSION,
    );
    (databaseService.getContractEventIndexerState as any).mockReturnValue({
      name: "soroban_event_indexer",
    });
    (databaseService.saveContractEventIndexerState as any).mockImplementation(
      (state: any) => ({
        name: "soroban_event_indexer",
        ...state,
      }),
    );
    (databaseService.resetContractEventIndexerState as any).mockImplementation(
      (fromLedger?: number) => ({
        name: "soroban_event_indexer",
    ...
    (contractEventIndexerService as any).contractAddress =
      "CDLZFC3SYJYDZT7K67VZ75HPJVIEWCEUNYQZ2QZ2QZ2QZ2QZ2QZ2QZ2Q";
    (contractEventIndexerService as any).status.contractAddress =
      "CDLZFC3SYJYDZT7K67VZ75HPJVIEWCEUNYQZ2QZ2QZ2QZ2QZ2QZ2QZ2Q";
  ...
  it("parses events matching the schema", async () => {
    ...
    const safeScValToNativeSpy = vi.spyOn(
      contractEventIndexerService as any,
      "safeScValToNative",
    );
    ...
    expect(databaseService.recordRebalanceEvent).toHaveBeenCalledWith(
  ...
  it("resumes from the durable cursor without guessing a start ledger", async () => {
    (databaseService.getContractEventIndexerState as any).mockReturnValue({
      name: "soroban_event_indexer",
      cursor: "stored-cursor",
      latestLedger: 998,
      lastSuccessfulSyncAt: "2023-01-01T00:00:00.000Z",
    });

    await contractEventIndexerService.syncOnce();

    expect(rpcServerMock.getLatestLedger).not.toHaveBeenCalled();
    expect(rpcServerMock.getEvents).toHaveBeenCalledWith(
      expect.objectContaining({
        cursor: "stored-cursor",
  ...
  it("persists page cursor state as pages complete", async () => {
    const safeScValToNativeSpy = vi.spyOn(
      contractEventIndexerService as any,
      "safeScValToNative",
    );
    (contractEventIndexerService as any).maxPagesPerSync = 2;
    ...
    rpcServerMock.getEvents
      .mockResolvedValueOnce({ events: [firstEvent], latestLedger: 1000 })
      .mockResolvedValueOnce({ events: [secondEvent], latestLedger: 1001 });

    safeScValToNativeSpy.mockImplementation((val) => {
      if (val === "topic1") return "portfolio";
      if (val === "topic2") return "rebalance_executed";
      if (val === "first-value") return ["portfolio-123", "user-abc"];
      if (val === "second-value") return ["portfolio-456", "user-def"];
      return undefined;
    });
    ...
    expect(databaseService.saveContractEventIndexerState).toHaveBeenCalledWith(
      expect.objectContaining({ cursor: "token-1", latestLedger: 1000 }),
      "soroban_event_indexer",
    );
    expect(databaseService.saveContractEventIndexerState).toHaveBeenCalledWith(
      expect.objectContaining({ cursor: "token-2", latestLedger: 1001 }),
      "soroban_event_indexer",
    );
    expect(
      databaseService.saveContractEventIndexerState,
