...
use portfolio_rebalancer::value_to_balance;
use portfolio_rebalancer::{ALLOCATION_DENOMINATOR, DEFAULT_ASSET_DECIMALS};
...
proptest! {
    ...
    fn property_valid_allocations_always_accepted(
        // Generate 1–9 parts, each limited so that the max total is < 10000
        // with high probability, reducing `prop_assume!` rejections.
        parts in proptest::collection::vec(1u32..=5000u32, 1..=9usize),
    ) {
