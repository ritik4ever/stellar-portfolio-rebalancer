...
fn log_tx_info(output: &Output, label: &str) {
    ...
    // Transaction hash: 64 hex characters — scan within tokens rather than
    // requiring the entire line to be the hash.
    for line in combined.lines() {
        for token in line.split_whitespace() {
            let token = token.trim_matches(|c: char| !c.is_ascii_hexdigit());
            if token.len() == 64 && token.chars().all(|c| c.is_ascii_hexdigit()) {
                eprintln!("[testnet-int]   {label} — tx hash: {token}");
                return;
            }
        }
    }
...
fn testnet_full_rebalance_lifecycle() {
    ...
    // Verify capabilities and version
    {
        ...
        assert!(version.contains("1"), "Contract version should be \"1\", got: {version}");
