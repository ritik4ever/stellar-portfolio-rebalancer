...
| Column | Purpose |
|-----|---------|
| `name` | State row name (`soroban_event_indexer`) |
| `cursor` | Soroban RPC paging token for incremental event fetch |
| `latest_ledger` | Last known ledger sequence from RPC response |
| `last_successful_sync_at` / `last_failed_sync_at` | Durable sync outcome timestamps for restarts and operators |
| `last_error` | Last actionable failure message surfaced by the cursor endpoint |
